...
router.post("/charge", auth0Verify, async (req, res) => {
  ...
  const currency = "usd"; // TODO IMPORTANT: CHANGE CURRENCY TO CAD ON PRODUCTION
  ...
  request(options, async function (error, response, body) {
    ...
    if (body && body.status != undefined && body.status === "succeeded") {
      try {
        ...
        let amountAfterDiscount = rawTot - totDiscounts;

        let grossTax = taxes.reduce((total, currentTax) => {
          return total + (amountAfterDiscount / 100) * currentTax.percentage;
        }, 0);

        let grossTotal = amountAfterDiscount + shipping + grossTax;
        ...
        let order = new Order({
          ...
          taxes: taxes.map((tax) => {
            return {
              ...
              amount: (tempSubTotal / 100) * tax.percentage,
            };
          }),
          shipping: shipping,
        });
